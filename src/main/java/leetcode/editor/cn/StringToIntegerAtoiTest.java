...
public class StringToIntegerAtoiTest {
    ...
    public static void main(String[] args) {
        Solution solution = new StringToIntegerAtoiTest().new Solution();
...
/*            String regex = "^\\s*([+-]?)0*([1-9]\\d*).*$";
            if (str.matches(regex)) {
                String s = str.replaceAll(regex, "$1+$2").replace("+", "");
                if (s.length() > 0) {
                    String min = String.valueOf(Integer.MIN_VALUE);
                    String max = String.valueOf(Integer.MAX_VALUE);
                    if (s.startsWith("-")) {
                        if (s.length() > min.length() || (s.length() == min.length() && s.compareTo(min) > 0)){
                            return Integer.MIN_VALUE;
                        }
                    } else {
                        if (s.length() > max.length() || (s.length() == max.length() && s.compareTo(max) > 0)) {
